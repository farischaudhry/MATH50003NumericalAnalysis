...
\textbf{Numerical Analysis MATH50003 (2022\ensuremath{\endash}23) Problem Sheet 1}

\rule{\textwidth}{1pt}
\textbf{Problem 1} Assuming $f$ is differentiable, prove the left-point Rectangular rule error formula
\[
\ensuremath{\int}_a^b f(x) {\rm d}x =  h \ensuremath{\sum}_{k=0}^{n-1} f(x_k) +  \ensuremath{\delta}
\]
...
\[
\ensuremath{\int}_a^b f(x) {\rm d}x = h \br[{f(a) \over 2} + \ensuremath{\sum}_{j=1}^{n-1} f(x_k) + {f(b) \over 2}] +  \ensuremath{\delta}
\]
...
\]
show that $|\ensuremath{\delta}| \ensuremath{\leq} Mh/2$ for $M = \sup_{x \ensuremath{\leq} t \ensuremath{\leq} x+h}\abs{f''(t)}$.
